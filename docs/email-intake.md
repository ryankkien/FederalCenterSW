...
Each committed attachment runs the same inline intake step as portal uploads. When
inline AI is enabled and extracted text is available, the LLM classifies the document
and suggests contract matches immediately; otherwise filename, email-subject-derived
title, notes, and configured document type provide fallback decisions before async
processing. The decisions are persisted to `document_classification_decisions` and
`document_match_decisions`; queued processing still runs the full AI-first analysis
path.
